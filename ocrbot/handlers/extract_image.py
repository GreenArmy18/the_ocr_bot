    ...
    if file_path is not None:
        ...
        if data['IsErroredOnProcessing']==False:
            if update.message.photo[-1].height== '1280' and update.message.photo[-1].width == '576':
                for x in range(len(data['ParsedResults'][0]['TextOverlay']['Lines'])):
...
def calculate(data_list):
    #t1,t2=timedelta(hours=1, milliseconds=0),timedelta(hours=1, milliseconds=0)
    total_time=timedelta(hours=1, milliseconds=0)-timedelta(hours=1, milliseconds=0)
    ...
    for x in range(1,5):
        ...
        #diff = time1 - time2
        #total_time+=(diff)
        total_time+= (datetime.strptime((re.sub('[^0-9]', '', str(hours[x*2-2]))),"%H%M") - datetime.strptime((re.sub('[^0-9]', '', str(hours[x+x-1]))),"%H%M"))
    test=total_time
